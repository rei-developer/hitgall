...
module.exports.getTopics = async ctx => {
  ...
  if (page < 0) return;
  if (limit < 5 || limit > 50) return;
  const obj = {};
  if (best > 0) obj.isBest = 1;
  if (domain === "best") obj.isBest = 1;
  ...
  if (notices.length > 0) {
    notices = notices.map(item => {
      ...
      if (ip && ip.length >= 3) item.ip = `${ip[0]}.${ip[1]}`;
      else item.ip = "";
      return item;
    });
  }
...
module.exports.getContent = async ctx => {
  ...
  if (ip && ip.length >= 3) topic.ip = `${ip[0]}.${ip[1]}`;
  else topic.ip = "";
  topic.header = "";
  const images = topic.isImage > 0 ? await readTopic.topicImages(id) : [];
  const boardLevel = user
    ? user.isAdmin < 1
      ? await readBoard.adminBoardManagerLevel(user.id, topic.boardDomain)
      : 3
    : 0;
...
module.exports.createTopic = async ctx => {
  ...
  if (domain === null || domain === "") domain = "anime";
  if (title === "" || content === "<p></p>") return;
  ...
  if (isExist) {
    ...
    if (days <= 0)
      return (ctx.body = {
        message: `현재 해당 갤러리에서 차단된 상태입니다. (${moment(
          isExist.blockDate
  ...
  if (isImage) await createTopic.createTopicImages(topicId, domain, images);
  if (user) {
...
module.exports.createTopicSave = async ctx => {
  ...
  let { isNotice, color, title, content } = ctx.request.body;
  if (title === "" && content === "<p></p>") return;
  title = Filter.disable(title);
  content = Filter.topic(content);
  if (color !== "") color = color.replace("#", "");
...
module.exports.createTopicVotes = async ctx => {
  ...
  if (likes) {
    if (topic.isBest === 0 && topic.likes - topic.hates >= BEST_LIMIT) {
      ...
      if (targetUser > 0) await User.setUpExpAndPoint(targetUser, 100, 100);
      // await socket.newBest(global.io, id, topic.boardDomain, topic.title)
    } else {
      if (targetUser > 0) await User.setUpExpAndPoint(targetUser, 5, 5);
    }
    ...
    if (topic.isBest === 1 && topic.hates - topic.likes >= BEST_LIMIT) {
      ...
      if (targetUser > 0) await User.setUpExpAndPoint(targetUser, -20, -20);
...
module.exports.createPostVotes = async ctx => {
  ...
  if (likes) await updatePost.updatePostCountsByLikes(id);
  else await updatePost.updatePostCountsByHates(id);
...
module.exports.updateTopic = async ctx => {
  ...
  if (user.isAdmin < 1 && topic.userId !== user.id) return;
  let {
    domain,
    isNotice,
    category,
  ...
  const regex = /<img[^>]*alt=[\"']?([^>\"']+)[\"']?[^>]*>/gim;
  let findImages = [];
  let xArray;
  while ((xArray = regex.exec(content))) findImages.push(xArray[1]);
  ...
  if (trashImages) {
    ...
    await Promise.all(jobs);
    const jobsForThumb = trashImages.map(
      image =>
        new Promise(async resolve => {
    ...
    const jobsForDB = trashImages.map(
      image =>
        new Promise(async resolve => {
  ...
  if (isImage) await createTopic.createTopicImages(id, domain, images);
  ctx.body = {
    topicId: id,
    status: "ok"
  };
...
module.exports.updatePost = async ctx => {
  ...
  if (user && user.isAdmin < 1 && userId !== user.id) return;
...
module.exports.deleteTopic = async ctx => {
  ...
  if (user && user.isAdmin > 0) await deleteTopic(id);
  else await updateTopic.updateTopicByIsAllowed(id);
  if (user && (level > 0 || user.isAdmin > 0))
    await createRemoveLog(
      user.id,
      topic.boardDomain,
      topic.author,
      topic.title,
